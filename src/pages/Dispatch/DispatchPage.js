...
function AssignmentComplete(guide, booking) {
  return `<section class="dispatch-complete"><span class="dispatch-complete__icon" aria-hidden="true">✓</span><p class="eyebrow">Assignment complete</p><h2>${guide.name} is ready to lead.</h2><p>${guide.name} has been assigned to ${booking.hiker}’s ${booking.trail} trek on ${booking.schedule}.</p><div>${StatusBadge({ label: 'Guide notified', tone: 'success' })}${StatusBadge({ label: 'Tourist notified', tone: 'success' })}</div>${Button({ label: 'Assign another booking', variant: 'secondary', attributes: 'data-dispatch-action="restart"' })}</section>`
}
