...
const createInitialState = () => ({ step: 0, trailId: '', scheduleId: '', packageId: '', guideId: '', confirmed: false })
...
export function changeBookingStep(direction) {
  bookingState = { ...bookingState, step: Math.min(Math.max(bookingState.step + direction, 0), bookingSteps.length - 1) }
}

export function canAdvanceBooking() {
  return Boolean([bookingState.trailId, bookingState.scheduleId, bookingState.packageId, bookingState.guideId][bookingState.step])
}
